...
func ResolveEndpoint(configPath string) (ResolvedEndpoint, error) {
	strategies := []struct {
		...
		{"Claude Code OAuth token", tryClaudeCodeOAuthEnv},
	...
	return ResolvedEndpoint{}, fmt.Errorf("no valid LLM endpoint configured; one of OCR_LLM_URL/OCR_LLM_TOKEN/OCR_LLM_MODEL, ~/.opencodereview/config.json, ANTHROPIC_BASE_URL/ANTHROPIC_AUTH_TOKEN/ANTHROPIC_MODEL, or CLAUDE_CODE_OAUTH_TOKEN with ANTHROPIC_MODEL must be set")
}
...
// tryClaudeCodeOAuthEnv reads a Claude Code long-lived OAuth token generated by
// `claude setup-token`. The token authenticates through the Anthropic Messages
// API with Claude Code's OAuth beta header.
func tryClaudeCodeOAuthEnv() (ResolvedEndpoint, bool, error) {
	token := os.Getenv(envClaudeCodeOAuthToken)
	model := os.Getenv(envCCModel)
	if model == "" {
		model = os.Getenv(envOCRLLMModel)
	}
